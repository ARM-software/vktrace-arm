...
BOOL vktrace_File_IsCompressed(FILE* fp) {
    // Check if the trace file is gzipped by checking (first_byte == 0x1f) && (second_byte == 0x8b)
    ...
    offset = Ftell(fp);
    rewind(fp);
    if (1 == fread(&firstByte, 1, 1, fp) && (1 == fread(&secondByte, 1, 1, fp))) {
    ...
    if (Fseek(fp, offset, SEEK_SET) != 0) {
        rewind(fp);
        vktrace_LogError("Failed to fseek to restore the position of tracefile for replaying.");
    }
...
BOOL vktrace_File_Decompress(const char* infile, const char* outfile) {
    ...
    if (in == NULL) {
        vktrace_LogError("Failed to open compressed tracefile to decompress.");
        return FALSE;
    }

    FILE* out = fopen(outfile, "wb");
    if (out == NULL) {
        vktrace_LogError("Failed to open temp file to write.");
        return FALSE;
    }

    vktrace_LogAlways("Decompressing trace file...");
    for (;;) {
        buflen = gzread(in, buf, sizeof(buf));
        if (buflen < (int)sizeof(buf)) {
            if (buflen < 0) {
                vktrace_LogError("gzread error: %s.", gzerror(in, &errnum));
                return FALSE;
            }
            if (buflen == 0) {
                // end of file
        ...
        if ((int)fwrite(buf, 1, (size_t)buflen, out) != buflen) {
            vktrace_LogError("Failed to write to temp file.");
            return FALSE;
        }
    }
    fflush(out);
    vktrace_LogAlways("Decompressing trace file...Done");

    if (fclose(out)) {
        vktrace_LogError("Failed to close temp file.");
        return FALSE;
    }

    if (gzclose(in) != Z_OK) {
        vktrace_LogError("Failed to close compressed tracefile.");
        return FALSE;
    }
    return TRUE;
}
