...
#############################################################
# Some apk needs to be run without trace the first time.

# install screenshot layer
adb $serialFlag shell rm -rf /data/local/debug/vulkan
adb $serialFlag shell mkdir -p /data/local/debug/vulkan
adb $serialFlag push libs/$target_abi/libVkLayer_screenshot.so /data/local/debug/vulkan/
...
adb $serialFlag shell rm -rf /sdcard/Android/1.ppm
...
sleep 5s # pause to let the screenshot write finish
until adb $serialFlag shell ls -la /sdcard/Android/1.ppm
do
    echo "Waiting for $package screenshot on $serial"

    if [ $(date +%s) -gt $vktrace_end_time ]
    then
        echo "$package timeout reached: $vktrace_seconds seconds"
        echo "No $package screenshot, closing down"
        # Cleanup
        adb $serialFlag shell am force-stop $package
...
# install trace layer and screenshot layer
adb $serialFlag shell rm -rf /data/local/debug/vulkan
adb $serialFlag shell mkdir -p /data/local/debug/vulkan
adb $serialFlag push libs/$target_abi/libVkLayer_vktrace_layer.so /data/local/debug/vulkan/
adb $serialFlag push libs/$target_abi/libVkLayer_screenshot.so /data/local/debug/vulkan/
...
# remove trace layer and screenshot layer
adb $serialFlag shell rm -rf /data/local/debug/vulkan

# Move <package>-1.vktrace to <package>.vktrace for IceCave
# FIXME: Find a smarter way to do this.
if [ -f ${package}-1.vktrace ]; then
    mv ${package}-1.vktrace $package.vktrace
fi
...
sleep 5s # small pause to allow permission to take

# Wake up the device
adb $serialFlag shell input keyevent "KEYCODE_MENU"
adb $serialFlag shell input keyevent "KEYCODE_HOME"

adb $serialFlag shell am start -a android.intent.action.MAIN -c android-intent.category.LAUNCH -n com.example.vkreplay/android.app.NativeActivity --es args "-v\ full\ -t\ /sdcard/$package.vktrace -pt true"
