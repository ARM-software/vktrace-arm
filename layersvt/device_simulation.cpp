...
const char *const kEnvarDevsimCompatMode = "debug.vulkan.devsim.compatmode";    // a non-zero integer will enable compatible mode.
#else
...
const char *const kEnvarDevsimCompatMode = "VK_DEVSIM_COMPAT_MODE";     // a non-zero integer will enable compatible mode.
#endif
...
VKAPI_ATTR VkResult VKAPI_CALL CreateInstance(const VkInstanceCreateInfo *pCreateInfo, const VkAllocationCallbacks *pAllocator,
    ...
    g_run_on_compat_mode = atoi(GetEnvarValue(kEnvarDevsimCompatMode).c_str());
    ...
    for (const auto &physical_device : physical_devices) {
        ...
        EnableCompatMode(*pInstance, physical_device, pdd);
    }
    ...
    if (src_count == 0) {
        ...
        if (g_run_on_compat_mode) {
            ...
            if (!pQueueFamilyProperties) {
                if (src_count < *pQueueFamilyPropertyCount) {
                    *pQueueFamilyPropertyCount = src_count;
                }
            } else {
                for (uint32_t i = 0; i < *pQueueFamilyPropertyCount; i++) {
                    ClampQueueFamilyProps(pdd->arrayof_queue_family_properties_[i], pQueueFamilyProperties[i]);
                }
            }
        } else {
...
VKAPI_ATTR void VKAPI_CALL GetPhysicalDeviceQueueFamilyProperties2KHR(VkPhysicalDevice physicalDevice,
    ...
    for (uint32_t i = 0; i < copy_count; ++i) {
        if (g_run_on_compat_mode) {
            ClampQueueFamilyProps(src_props[i], pQueueFamilyProperties2[i].queueFamilyProperties);
        } else {
...
VKAPI_ATTR void VKAPI_CALL GetPhysicalDeviceFormatProperties(VkPhysicalDevice physicalDevice, VkFormat format,
    ...
    if (src_count == 0) {
        ...
        if (iter == pdd->arrayof_format_properties_.end()) {
            *pFormatProperties = VkFormatProperties{};
        } else if (g_run_on_compat_mode) {
            dt->GetPhysicalDeviceFormatProperties(physicalDevice, format, pFormatProperties);
            ClampFormatProps(iter->second, *pFormatProperties);
        } else {
