...
class GoodRepo(object):
    ...
    def Checkout(self):
        ...
        if "Vulkan-Headers" in self.repo_dir:
            if self._args.arch == '32' or self._args.arch == 'x86' or self._args.arch == 'win32':
                padding_i386_file = os.path.join(sys.path[0], "../vulkan_structs_padding_i386.h")
                os.system("cat {} >> {}/include/vulkan/vulkan_core.h".format(padding_i386_file, self.repo_dir))
    ...
    def CMakeConfig(self, repos):
        ...
        if platform.system() == 'Windows':
            ...
            cmakedir = os.path.join(sys.path[0], "../cmake/")
            if self._args.arch == 'arm_32':
                print("Building for arm_32")
...
def main():
    ...
    parser.add_argument(
        '--arch',
        dest='arch',
        choices=['arm_32', 'arm_64', '32', '64', 'x86', 'x64', 'win32', 'win64'],
        type=str.lower,
